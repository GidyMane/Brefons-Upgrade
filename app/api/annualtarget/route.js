...
export async function POST(req, res) {
  ...
  let { data: annual_target, error: err } = await supabase
    .from("Counties")
    .select(
      "*,annual_target(id,sub_indicator(id,indicator_name,Category(id,category,Sub_category(id,sub_category)),Indicators(id,IndicatorName,Sub_components(id,component_name,Components(id,ComponentName)))))"
    )
    .eq("id", county_id)
    .filter("annual_target.date", "eq", date);

  const renameAnnualTargetToActivities = (dataArray) => {
    return dataArray.map((item) => {
      const { annual_target, ...rest } = item; // Destructure to separate annual_target from other properties
      return { ...rest, Activities: annual_target }; // Create a new object with the renamed key
    });
  };

  const updatedData = renameAnnualTargetToActivities(annual_target);
