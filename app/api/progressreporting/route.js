...
export async function POST(req, res) {
  ...
  let { data: progress_reporting, error: err } = await supabase
    .from("Counties")
    .select(
      "*,progress_reporting(id,sub_indicator(id,indicator_name,Category(id,category,Sub_category(id,sub_category)),Indicators(id,IndicatorName,Sub_components(id,component_name,Components(id,ComponentName)))))"
    )
    .eq("id", county_id)
    .filter("progress_reporting.date", "eq", date);

  const renameAnnualTargetToActivities = (dataArray) => {
    return dataArray.map((item) => {
      const { progress_reporting, ...rest } = item; // Destructure to separate annual_target from other properties
      return { ...rest, Activities: progress_reporting }; // Create a new object with the renamed key
    });
  };

  const updatedData = renameAnnualTargetToActivities(progress_reporting);
  let data;
